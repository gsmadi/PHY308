...
int at=-1; // active trail number, -1 means not used
void save_config(void);
void load_config(void);

void trail_add(int at, double x, double y, double z);
...
//   printf("mouse coords % g, % g, % g; guess coords % g, % g, % g\n",mx,my,mz,x,y,z);
  ...
  else if (line[0] == 'F' && line[1] == 'P' && line[2] == 'S')
  {
  ...
  else if (line[0] == '@' && line[1] == 'l') { // line, no color specified
    sscanf(&line[3],"%d %f %f %f %f",&at,&x1,&y1,&x2,&y2);
    trail_add(at,x1,y1,0.0);
  ...
  else if (line[0] == '@' && line[1] == 'c' && line[2] == '3' && line[3] == ' ') { // 3d circle
    td=1;
    sscanf(&line[3],"%d %f %f %f %f",&at,&xc1,&yc1,&zc1,&r);
    trail_add(at,xc1,yc1,zc1);
    transform(xc1,yc1,zc1,&x,&y,&z);
    r = r/(z+vdist)*(vdist/scale);
    ...
    glBegin(GL_LINES);
    lin3(xc1,yc1,zc1,xc2,yc2,zc2);
    glEnd();
 }

  else if (line[0] == '@' && line[1] == 'l' && line[2] == '3' && line[3] == ' ') { // 3d line
    td=1;
    sscanf(&line[4],"%d %f %f %f %f %f %f",&at,&xc1,&yc1,&zc1,&xc2,&yc2,&zc2);
    trail_add(at,xc1,yc1,zc1);
    glBegin(GL_LINES);
    lin3(xc1,yc1,zc1,xc2,yc2,zc2);
    glEnd();
...
//    y = y/(z+vdist)*(vdist/scale);
  ...
  else if (line[0] == '@' && line[1] == 'c' && line[2] == ' ') { // circle
    sscanf(&line[3],"%d %f %f %f",&at,&x1,&y1,&r);
    trail_add(at,x1,y1,0);
...
void keyb(unsigned char key, int x, int y)
{
  ...
  if (key == 'I') {inverse=1-inverse; glClearColor(inverse, inverse, inverse, 1.0); if (inverse) contrast*3; else contrast/3;}
...
void trail_add(int n, double x, double y, double z)
{
  trail_buf[n]=(trail_buf[n]+1)%LTRAILS;
  printf("Adding point %e, %e, %e to trail %d; ring buffer now %d\n",x,y,z,n,trail_buf[n]);
  trailx[n][trail_buf[n]]=x;
  traily[n][trail_buf[n]]=y;
  trailz[n][trail_buf[n]]=z;
}
  

int main(int argc, char **argv)
{
  trailx=(double **)malloc(sizeof(double *)*NTRAILS);
  traily=(double **)malloc(sizeof(double *)*NTRAILS);
  trailz=(double **)malloc(sizeof(double *)*NTRAILS);

  int i;
  for (i=0;i<NTRAILS;i++)
  {
    trail_buf[i]=0;
    trailx[i]=(double *)malloc(sizeof(double)*LTRAILS);
    traily[i]=(double *)malloc(sizeof(double)*LTRAILS);
    trailz[i]=(double *)malloc(sizeof(double)*LTRAILS);
  }
