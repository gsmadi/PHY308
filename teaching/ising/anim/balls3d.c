...
int check_zonelist(item *zonelist[NG][NG],int xzone[N],int yzone[N],int zzone[N],double x[N],double y[N], double z[N])
{
  static int wrong;
  wrong=0;
//  printf("!Start update\n");
  static int i,xx,yy,zz;

  static item *curr, *head, *curr2;
...
//    printf("!Ball %d is in box %d,%d, should be in %d,%d\n",i,xzone[i],yzone[i],xx,yy);
     del(&zonelist[xzone[i]][yzone[i]][zzone[i]],i);
...
main()
{
  ...
  int ehisto[NH];
  int ehisto2[NH];
  double x[N],y[N],z[N],vx[N],vy[N],vz[N],,mass[N],R[N];
  int j,k,l,m,n,s,vbin,ebin,zx,zy,zz;
  double dx,dy,dz;
  ...
  int check=0,collide=0;
  for (i=0;i<N;i++) 
  {
    xzone[i]=0; yzone[i]=0; zzone[i]=0;
    mass[i]=1;
    if (i % 20 == 0) mass[i]=5;
   R[i]=sqrt(mass[i])*0.002;
    ...
    for (j=0;j<i;j++)
    {
      if (hypot3(x[i]-x[j],y[i]-y[j],z[i]-z[j]) < R[i]+R[j])
      {
         x[i]=drand48()-.5;
         z[i]=drand48()-.5;
         y[i]=(drand48()-.5)/2-0.5;
  ...
  for (t=-1;1;t+=dt)
  {
    ...
    for (i=0;i<N;i++)
    {
      ...
      dy=(vy[i]+g*0.5*dt)*dt;
      dz= vz[i]*dt;
      if (hypot(dx,dy) > R[i]*.4) {printf("!ACK: might have skipped a collision (dx,dy = %e max is %e)\n",hypot(dx,dy),R[i]*.4);}
      x[i] += dx;
      y[i] += dy; 
      z[i] += dz; 
      vy[i] += g*dt;
...
// Collision checking
  int m=check_zonelist(zonelist,xzone,yzone,zzone,x,y,z);
   check=collide=0;
    for (i=0;i<N;i++)
    {
// We only need to check for collisions if they are in the same zone or adjacent
      ...
      zy=getzone(y[i]);
      zz=getzone(z[i]);
      for (k=zx-1;k<=zx+1;k++)       for (l=zy-1;l<=zy+1;l++)     for (m=zz-1; m<=zz+1;m++)
      {
        if (k<0 || k>=NG || l<0 || l>=NG || m<0 || m>=NG) continue;
        curr=zonelist[k][l][m];
        while (curr)
        {
        ...
        check++;
        if ((x[i]-x[j])*(x[i]-x[j]) + (y[i]-y[j])*(y[i]-y[j]) + (z[i]-z[j])*(z[i]*z[j]) < (R[i]+R[j])*(R[i]+R[j]) && (vx[i]-vx[j])*(x[i]-x[j]) + (vy[i]-vy[j])*(y[i]-y[j]) + (vz[i]-vz[j])*(z[i]-z[j]) < 0)
        {
           collide++;
...
//          printf("lC %f %f %f %f 1 0 0 \n",x[i],y[i],x[j],y[j]);
          ...
          vzcom = (vz[i]*mass[i]+vz[j]*mass[j])/(mass[i]+mass[j]);
...
*/
 ...
 /*
  ...
  printf("T -.95 0.85\nTime: %f \t %d/%d collisions \t %d in wrong zones\n",t,check,collide,m);
  }
